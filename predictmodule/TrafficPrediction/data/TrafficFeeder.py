...
data = pd.Series.from_csv("data/10min_workload.csv",
...
class TrafficFeeder():
    ...
    def normalize(self, dataCount):
        dataNorm = (dataCount - dataCount.min()) / \
            (dataCount.max() - dataCount.min())
        return dataNorm
    ...
    def generate(self, range_training):
        ...
        self.workload = data[142 * range_training[0] -
                             self.n_input:142 * range_training[1]]
        data_training = self.normalize(self.workload)
        X_training = self.getTraining(self.workload)
        data_test = self.normalize(
            data[142 * range_training[0]:142 * range_training[1]])
    ...
    def getTraining(self, workload):
        ...
        for t in range(self.n_input, n_row):
            ...
            for j in range(1, self.n_periodic + 1):
                ...
                norVal = (raw_data[start_idx - 142 * j] -
