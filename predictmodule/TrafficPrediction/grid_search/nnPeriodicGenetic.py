...
class NNGridSearch:
    ...
    def gridsearch_alpha(self, learning_rate, index, params=None):
        ...
        self.l_in = ls.layers.InputLayer(
            shape=(None, n_input), input_var=None, W=params.T)
        ...
        for item in learning_rate:
            ...
            name_file = "GeneticParams/saveNeuralNetwork_%s_%s.tdn" % (
                item, index)
...
list_results = np.zeros(
    [list_ninput.shape[0], learning_rate.shape[0]], dtype=np.float64)
i = list_ninput[0]
...
# In[44]:
print "Saving data..."
storeResult["results_gn_periodic_grid"] = pd.DataFrame(
    list_results, index=list_ninput, columns=learning_rate)
storeResult.close()
# ax = pl.subplot()
