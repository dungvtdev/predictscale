...
class FeedFlow:
    ...
    def __init__(self, neural_shape=None, uniform_init=True, activation=None):
        ...
        self.W, self.b = self.initialize_params(
            self.weight_layers, self.bias_layers)
    ...
    def flow(self, X):
        ...
        pipe_activator = [self.activation for t in range(
            self.number_of_layers - 1)]
        pipe_activator[-1] = None
        input_flow = X
        output = []
        for (index, param) in enumerate(param_network):
            output = self.activator(input_flow, param[0], param[
                                    1], pipe_activator[index])
            input_flow = output
        return output
    ...
    def construct_candidate(self):
        ...
        for param in param_network:
            temp_weight = np.concatenate(
                [param[0].flatten(), param[1].flatten()])
