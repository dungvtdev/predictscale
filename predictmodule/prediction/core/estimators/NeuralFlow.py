...
class NeuralFlowRegressor(BaseEstimator):
    ...
    def model_regression(self, X, y):
        input_layer = tf.contrib.layers.fully_connected(
            X, self.n_hidden, activation_fn=tf.nn.relu, weight_init=self.weight_init, bias_init=self.bias_init)
        ...
        pred = tf.contrib.layers.fully_connected(
            input_layer, self.n_output, weight_init=self.weight_init, bias_init=self.bias_init)
    ...
    def fit(self, X, y, **param):
        ...
        self.bias_layers = [self.neural_shape[t]
                            for t in range(1, len(self.neural_shape))]
        ...
        if(self.weights_matrix == None):
            self.W, self.b = initialize_param(
                self.weight_layers, self.bias_layers, self.uniform_init)
        else:
        ...
        self.X = tf.placeholder(
            "float", [None, self.neural_shape[0]], name="input")
        self.y = tf.placeholder(
            "float", [None, self.neural_shape[-1]], name="output")
